...
sys.stdin = open('input_p13_guard.txt')
...
# 비효율적인 풀이
    ...
    i = 1
    while guard_c - i >= 0 and space[guard_r][guard_c - i] == 0:
        blind_spot -= 1
        i += 1
    i = 1
    while guard_c + i < N and space[guard_r][guard_c + i] == 0:
        blind_spot -= 1
        i += 1
    i = 1
    while guard_r - i >= 0 and space[guard_r - i][guard_c] == 0:
        blind_spot -= 1
        i += 1
    i = 1
    while guard_r + i < N and space[guard_r + i][guard_c] == 0:
        blind_spot -= 1
        i += 1
